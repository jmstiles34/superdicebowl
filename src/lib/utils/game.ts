...
export function inFieldGoalRange(action: string, possession: string, ballIndex: number) {
	return (
		showFieldGoalPulse(action) && compareFns[possession](ballIndex, BALL_FIELD_GOAL[possession])
	);
}
...
export function madeFirstDown(
	...
	return (
		autoFirstDown || (!equals(firstDownIndex, -1) && compareFns[pos](ballIndex - 1, firstDownIndex))
	);
}
